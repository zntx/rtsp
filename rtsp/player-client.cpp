...
PlayerClient::PlayerClient(std::string path)
{
...
observable.Observable PlayerClient::Start() 
{
	return observable.Start(func() interface{} {
		l, err := url.Parse(this->Path)
		if err != nil {
			return err
		}
		...
		this->Conn = &conn
		this->connRW = bufio.NewReadWriter(bufio.NewReaderSize(conn, 10240), bufio.NewWriterSize(conn, 10240))
		...
		resp, err = this->Request("SETUP", headers)
		if err != nil {
			return err
		}
...
(resp *Response, err error) Request(std::string method, std::map<std::string,std::string> headers );
{
	...
	builder.Reset()
	for !this->Stoped {
		if line, _, err = this->connRW.ReadLine(); err != nil {
			return
		} else {
			if len(line) == 0 {
				fmt.Println("S->C	<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
